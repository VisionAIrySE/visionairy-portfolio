...
function renderOutreachChannels(m,forms){
 ...
 const companyForm=forms('company-channels',`${hidden('prospectId',p.id)}<label>Company name<input name="name" value="${esc(companyName)}" required></label><label>Company website<input name="website" type="url" value="${esc(website)}" placeholder="https://company.com"></label>${website?`<p><a href="${esc(website)}" target="_blank" rel="noopener">Open company website</a></p>`:''}<label>Company address<input name="address" value="${esc(companyAddress)}" placeholder="Street, city, state, ZIP"></label><label>Company inbox<input name="email" type="email" value="${esc(p.emailManualValue||p.email||'')}" placeholder="office@company.com"></label><label>Company phone<input name="phone" value="${esc(companyPhone)}" placeholder="Company phone"></label><label>Company LinkedIn page<input name="linkedInUrl" value="${esc(p.linkedInUrl||'')}" placeholder="https://www.linkedin.com/company/..."></label><button type="submit">Save company details</button>`);
