...
function renderOutreachChannels(m,forms,productId){
 const p=m.prospect;const activities=m.activities||[];const companyName=p.nameManualValue||p.name;const companyAddress=p.addressManualValue||p.address||'';const companyPhone=p.phoneManualValue||p.phone||'';const website=p.websiteManualValue||p.website||'';
 const companyLink=p.linkedInUrl?`<a href="${esc(p.linkedInUrl)}" target="_blank" rel="noopener">Open company LinkedIn page</a>`:`<a href="${esc(linkedInSearch('companies',companyName))}" target="_blank" rel="noopener">Find company on LinkedIn</a>`;
 const type=m.companyType||'NEEDS_REVIEW';const typeOption=(value,label)=>`<option value="${value}" ${type===value?'selected':''}>${label}</option>`;
 const typeField=productId==='stockerai'?`<label>Company type<select name="companyType">${typeOption('NEEDS_REVIEW','Needs review')}${typeOption('OPERATOR','Vending operator')}${typeOption('MACHINE_VENDOR','Machine sales or leasing vendor')}${typeOption('BOTH','Both operator and machine vendor')}</select></label>`:'';
 const companyForm=forms('company-channels',`${hidden('prospectId',p.id)}<label>Company name<input name="name" value="${esc(companyName)}" required></label>${typeField}<label>Company website<input name="website" type="url" value="${esc(website)}" placeholder="https://company.com"></label>${website?`<p><a href="${esc(website)}" target="_blank" rel="noopener">Open company website</a></p>`:''}<label>Company address<input name="address" value="${esc(companyAddress)}" placeholder="Street, city, state, ZIP"></label><label>Company inbox<input name="email" type="email" value="${esc(p.emailManualValue||p.email||'')}" placeholder="office@company.com"></label><label>Company phone<input name="phone" value="${esc(companyPhone)}" placeholder="Company phone"></label><label>Company LinkedIn page<input name="linkedInUrl" value="${esc(p.linkedInUrl||'')}" placeholder="https://www.linkedin.com/company/..."></label><button type="submit">Save company details</button>`);
 const contacts=(p.contacts||[]).filter(c=>!c.setAsideAt&&c.name).map(contact=>{
  const drafts=C.linkedinDrafts({...p,name:companyName},contact);const search=linkedInSearch('people',contact.name+' '+companyName);
  ...
  const details=forms('contact-channels',`${hidden('prospectId',p.id)}${hidden('contactId',contact.id)}<label>Name<input name="name" value="${esc(contact.name||'')}" required></label><label>Role<input name="role" value="${esc(contact.role||'')}" placeholder="Owner, Operations Manager, Driver..."></label><label>Email<input name="email" type="email" value="${esc(contact.email||'')}" placeholder="person@company.com"></label><label>Direct phone<input name="phone" value="${esc(contact.phone||'')}" placeholder="Direct phone"></label><label>LinkedIn profile<input name="linkedIn" value="${esc(contact.linkedIn||'')}" placeholder="https://www.linkedin.com/in/..."></label><button type="submit">Save contact details</button>`);
 ...
 const add=forms('contact-add',`${hidden('prospectId',p.id)}<h3>Add a person</h3><label>Name<input name="name" required placeholder="Name"></label><label>Role<input name="role" placeholder="Role"></label><label>Email<input name="email" type="email" placeholder="person@company.com"></label><label>Direct phone<input name="phone" placeholder="Direct phone"></label><label>LinkedIn profile<input name="linkedIn" placeholder="https://www.linkedin.com/in/..."></label><button type="submit">Add person</button>`);
...
function wireForm(form){
 ...
 form.saveChanges=async event=>{
  ...
  try {
   ...
   if(result.refresh){if(!dirty.size){location.reload();return;}const link=document.createElement('a');link.href=location.href;link.textContent=' Refresh to see saved details';status.append(link);}
