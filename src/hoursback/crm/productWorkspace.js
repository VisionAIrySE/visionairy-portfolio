...
function renderWorkspace(model,{csrf=''}={}) {
  ...
  return `<main><h1>${esc(product.name)}</h1><p class="notice">${sendState}</p><p>${count} companies in this product</p>${preparation?`<section aria-label="Whole product research progress"><h2>Preparation overview</h2><p>${preparation.researched} of ${preparation.total} active companies fully researched · ${preparation.unresearched} still need research.</p><p>${preparation.withEmail} have an email address on file; ${preparation.researchedWithEmail} of those are researched. Addresses still require review. ${preparation.archived} companies are archived.</p></section>`:''}<aside class="bulk-save"><button type="button" id="save-recipient-choices">Save all recipient choices</button><p id="bulk-status" role="status">Only changed recipient choices on this page are saved.</p></aside>${companies||'<p>No companies imported for this product yet.</p>'}${page>1?`<a href="${base}?page=${page-1}">Previous</a>`:''} ${page*25<count?`<a href="${base}?page=${page+1}">Next</a>`:''}</main>`;
}
