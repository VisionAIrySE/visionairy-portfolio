...
test('StockerAI phone and LinkedIn work stays scoped and a LinkedIn reply stops only that product campaign',async()=>{
 ...
 try{await db.$transaction(async tx=>{
  ...
  await C.saveCompanyChannels(scope,{...base,name:'Cascade Vending & Coffee',address:'10 Main St, Portland, OR',website:'cascadevending.example/routes',phone:'503-555-0100',linkedInUrl:'https://linkedin.com/company/cascade-vending',companyType:'BOTH'});
  ...
  assert.equal((await tx.productProspect.findUnique({where:{id:stock.member.id}})).companyType,'BOTH');
