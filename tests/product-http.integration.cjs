...
test('authenticated product routes save choices, reject bad tokens, and preserve legacy choices',async()=>{
 ...
 try{
  ...
  await db.productMessage.deleteMany({where:{productId:id}});await db.productEnrollment.deleteMany({where:{productId:id}});await db.productRecipient.deleteMany({where:{productId:id}});await db.productProspect.deleteMany({where:{productId:id}});await db.productSequence.deleteMany({where:{productId:id}});await db.cRMProduct.deleteMany({where:{id}});if(p)await db.prospect.delete({where:{id:p.id}});if(other)await db.prospect.delete({where:{id:other.id}});await db.$disconnect();
 }
});
