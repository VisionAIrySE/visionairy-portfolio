...
for (const line of fs.readFileSync(path.resolve(__dirname, '../../.env'), 'utf8').split(/\r?\n/)) {
  ...
  // Otherwise a revoked key in Windows silently overrides the replacement
  // Russ saved specifically for this CRM.
...
(async () => {
  if (!Number.isFinite(limit) || !Number.isFinite(expected)
      || !Number.isFinite(spendingCeiling) || spendingCeiling < 0) {
    throw new Error('Batch limit, expected count, or spending ceiling is invalid.');
  }
  ...
  // Three separately metered stages share the one ceiling Russ approved.
  // Each receives one third, so their combined possible spend cannot exceed it.
  const stageCeiling = spendingCeiling / 3;
  const finder = makeOpenRouterPool({
    model: 'google/gemini-3.1-flash-lite', apiKey: process.env.OPENROUTER_API_KEY,
    ceilingUsd: stageCeiling, hardKillMs: 120000, maxTokens: 1800, temperature: 0,
  ...
  const writer = makeOpenRouterPool({
    ...
    ceilingUsd: stageCeiling, hardKillMs: 120000, maxTokens: 1000, temperature: 0.2,
    ...
    try {
      ...
      if (readyIds.length) {
        const writerCode = await run('write-the-whole-sequence.cjs', [
          ...
          `--openrouter-ceiling=${stageCeiling}`,
        ]);
