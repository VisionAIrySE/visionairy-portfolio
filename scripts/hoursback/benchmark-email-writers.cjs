...
try {
  for (const line of fs.readFileSync('.env', 'utf8').split('\n')) {
    ...
    if (match) {
      localSettings[match[1]] = match[2];
      process.env[match[1]] = match[2];
    }
  }
} catch { /* environment may already be configured */ }
const OPENROUTER_KEY = localSettings.OPENROUTER_API_KEY || process.env.OPENROUTER_API_KEY;
const KEY_FINGERPRINT = OPENROUTER_KEY
  ? require('crypto').createHash('sha256').update(OPENROUTER_KEY).digest('hex').slice(0, 12)
  : 'missing';
const KEY_SOURCE = localSettings.OPENROUTER_API_KEY ? 'private project settings' : 'inherited environment';
...
(async () => {
  if (!OPENROUTER_KEY) throw new Error('OpenRouter access is not configured');
  console.log(`OpenRouter key fingerprint: ${KEY_FINGERPRINT} (${KEY_SOURCE})`);
  ...
  const results = [];
  await Promise.all(MODELS.flatMap((model) => samples.map(async ({ prospect, recipient, jobs }) => {
      try {
        ...
        results.push({ model, company: prospect.name, role: recipient.role, pass: judged.ok, why: judged.ok ? null : judged.why, cost: response.cost, ms: response.ms, subject: answer.subject, passage: answer.body, question: answer.question, letter, greeting, dayZero: judged.passage || answer.body });
      } catch (error) {
  ...
  const finalists = ['anthropic/claude-sonnet-5', 'openai/gpt-5.6-luna'];
  const followResults = [];
  await Promise.all(finalists.flatMap((model) => samples.flatMap(({ prospect, recipient, jobs }) => {
    const first = results.find((result) => result.model === model && result.company === prospect.name);
    if (!first || !first.letter) return [];
    return [2, 3, 4].map(async (touch) => {
      try {
        const response = await ask(model, W.askFor({
          name: prospect.name,
          trade: prospect.trade,
          roleTitle: recipient.role,
          jobs,
          otherJobs: [],
          dayZero: first.dayZero,
          touch,
          why: null,
        }));
        const full = W.buildLetter(touch, {
          greeting: first.greeting,
          passage: response.answer.body,
          seed: prospect.name,
        });
        const judged = J.judgeLetter(full, { day: W.THE_ANGLES[touch].day, jobs, roleTitle: recipient.role });
        followResults.push({ model, company: prospect.name, touch, pass: judged.ok, why: judged.ok ? null : judged.why, cost: response.cost, ms: response.ms, passage: response.answer.body });
      } catch (error) {
        followResults.push({ model, company: prospect.name, touch, pass: false, why: error.message });
      }
    });
  })));

  console.log('\nFULL-SEQUENCE FINALISTS (follow-ups only)');
  for (const model of finalists) {
    const rows = followResults.filter((result) => result.model === model);
    const modelCost = rows.reduce((sum, row) => sum + Number(row.cost || 0), 0);
    const timed = rows.filter((row) => row.ms);
    const averageMs = timed.length ? Math.round(timed.reduce((sum, row) => sum + row.ms, 0) / timed.length) : 0;
    console.log(`${model}: ${rows.filter((row) => row.pass).length}/${rows.length} follow-ups passed | $${modelCost.toFixed(4)} | ${averageMs} ms average`);
    for (const row of rows.filter((result) => !result.pass)) {
      console.log(`  FAIL ${row.company} day ${W.THE_ANGLES[row.touch].day} — ${row.why}`);
    }
  }
